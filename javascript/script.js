...
document.addEventListener("mousemove", (e) => {
  gsap.to(cursor, {
    x: e.clientX - cursor.offsetWidth * 0.5,
    y: e.clientY - cursor.offsetHeight * 0.5,
    duration: 0.5,
    ease: "power2.out",
  });
});
...
document.addEventListener("click", function (event) {
  ...
  const itemType = Math.random() < 0.5 ? "image" : "image";
  ...
  if (itemType === "video") {
    ...
    container.innerHTML = `<div class="img-container">
                                 <img src="./assets/svg-${imgNumber}.svg" alt="" style="width: 50%; height: 50%;"/>
                               </div>`;
  }
